...
export function registerQueryData(server: McpServer, env?: QueryEnv) {
	...
	server.registerTool(
		"gprofiler_query_data",
		{
			title: "Query Staged g:Profiler Data",
			description:
				"Query staged g:Profiler data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
			inputSchema: {
