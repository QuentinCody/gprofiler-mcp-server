...
import { GprofilerDataDO } from "./do";

export { GprofilerDataDO };

interface GprofilerEnv {
...
export class MyMCP extends StatelessMcpWorker {
	...
	async init() {
		...
		const env = this.env as unknown as GprofilerEnv;
...
export default {
	fetch(request: Request, env: Env, ctx: ExecutionContext) {
		...
		if (url.pathname === "/health") {
			return buildHealthResponse("gprofiler");
		}
		...
		if (url.pathname === "/readyz") {
			...
			return MyMCP.readiness(env, "gprofiler");
		}
